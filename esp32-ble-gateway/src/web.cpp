...
// Task to generate certificate with larger stack
static volatile bool certGenDone = false;
static volatile int certGenResult = 0;

static void certGenTask(void *param) {
  SSLCert **certPtr = (SSLCert **)param;
  
  Serial.println("Generating new HTTPS certificate (in task)");
  ...
  *certPtr = new SSLCert();
  ...
  certGenResult = createSelfSignedCert(
      **certPtr,
      KEYSIZE_2048,
      dn,
      "20190101000000",
...
bool WebManager::initCertificate()
{
  ...
  if (cert == nullptr)
  {
    // Run certificate generation in a separate task with larger stack
    certGenDone = false;
    certGenResult = 0;
    ...
    if (certGenResult != 0)
    {
      Serial.printf("Creating certificate failed. Error Code = 0x%02X, check SSLCert.hpp for details", certGenResult);
      return false;
    }
